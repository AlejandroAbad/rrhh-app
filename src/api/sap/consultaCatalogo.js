...
export const consultaCatalogo = async (redux, abortController, patronBusqueda, pagina = 1, limit = 50) => {
	...
	let werks = redux.getState().api.usuario.datos.werks;
	let consulta = {
		free_find: patronBusqueda
	}

	let url = `/api/zfm_web_materials/buscador/10100868/${werks}/?query=${JSON.stringify(consulta)}&page=${pagina}&result_page=${limit}`;
